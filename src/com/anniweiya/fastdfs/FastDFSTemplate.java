...
public class FastDFSTemplate {
    ...
    public FileInfo query_file_info(String group_name, String remote_filename) throws FastDFSException {
        StorageClient client = getClient();
        try {
            return client.query_file_info(group_name, remote_filename);
        }catch (Exception e) {
    ...
    public FastDfsInfo upload(InputStream inputStream, String ext,long size) throws FastDFSException {
        return this.upload(inputStream, ext, size,null);
    }

    public FastDfsInfo upload(InputStream inputStream, String file_ext_name,long size,
                              NameValuePair[] meta_list) throws FastDFSException {
        StorageClient client = getClient();

        final byte cmd = ProtoCommon.STORAGE_PROTO_CMD_UPLOAD_FILE;

        try {
            String[] uploadResults = client.do_upload_file(cmd, "group1", null, null, file_ext_name,
                    size, new UploadStream(inputStream, size), meta_list);
            ...
            fastDfsInfo.setFileInfo(fileInfo);
            return fastDfsInfo;
        } catch (Exception e) {
            throw new FastDFSException(e.getMessage(), e, 0);
        } finally {
